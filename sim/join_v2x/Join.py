...
#  sumo-gui -c sumocfg/join.sumocfg --remote-port 1337
# - Start the script:
#  python Join.py
#
##############################################################################
...
def custom_code_at_step_class_rsu() -> None:
	global vehicles, min_gap, tau, mode, Infras
...
def run_simulation() -> None:
	...
	traci.init(1337)
	Infras["CCTV"] = CCTV(GlobalSim.step, "CCTV", rsu_location, (5,25), "E1")
	while traci.simulation.getMinExpectedNumber() > 0:
		traci.simulationStep()  # Advance the simulation by one step
		custom_code_at_step_class_rsu()
		GlobalSim.step += 1
		colliding_number = traci.simulation.getCollidingVehiclesNumber()
		col += colliding_number
		# traci.polygon.addDynamics("car_area", "C_VEH00", (0, 2, 5, 9, 14))
		if is_visible == 0:
