...
#  sumo-gui -c sumocfg/intersection.sumocfg --remote-port 1337
# - Start the script:
...
from Infra   import Infra, CCTV, TrafficLight
import GlobalSim

num_c_veh = 0
accum_waiting_time = 0
...
def get_data(vehicle_id) -> Union[T_CDA, E_CDA, C_VEH, CE_VEH, N_VEH]:
	...
	if the_vehicle is None:
		...
		if vehicle_type[:5] == "C-VEH" or vehicle_type[:6] == "CE-VEH":
			the_vehicle.update(vehicle_position, vehicle_speed, vehicle_acceleration, vehicle_lane, vehicle_route, vehicle_edge, vehicle_route_index, vehicle_waiting_time, vehicle_accumulated_waiting_time)
...
def custom_code_at_step_class_rsu() -> None:
	global vehicles, min_gap, tau, mode, Infras, num_c_veh, accum_waiting_time

	# Add your custom code here
	vehicle_ids = traci.vehicle.getIDList()
	# print("Step({}) Vehicle IDs at step :{}".format(GlobalSim.step, vehicle_ids))
	
	# for infra_id, infra in Infras.items():
	# 	# print(f"infra id : {infra_id}, tpye : {infra.infra_type}")
	# 	if infra.infra_type == Infra.Type.CCTV:
	# 		infra.detection()
	# 		infra.send_detect_message(GlobalSim.step, rsu)
	...
	for vehicle_id in vehicle_ids:
		...
		if the_vehicle is not None and the_vehicle is not N_VEH:
			...
			elif the_vehicle.vehicle_type == "T-CDA":
				the_vehicle.send_bsm_plus(GlobalSim.step, rsu)
	...
	for tl_id in TL_edges:
		tl = TL_edges[tl_id]
		tl.receive(GlobalSim.step, rsu)
		# tl.set_state(GlobalSim.step)
	rsu.reset()
	...
	for vehicle_id in vehicles:
		...
		if v.stay == False:
			...
			if v.vehicle_type == "C-VEH":
				print(f"{v.vehicle_id} is arrived")
				print(f"accumulated waiting time :{v.vehicle_accum_wait_time}")
				accum_waiting_time += v.vehicle_accum_wait_time
				num_c_veh += 1 
				print(f"total step{GlobalSim.step}, number c-vdh: {num_c_veh}, accum_waiting_time: {accum_waiting_time}, mean: {accum_waiting_time/num_c_veh}")
...
def get_tl():
	...
	print(f"len: {len(traffic_lights)}")

	for traffic_light_id in traffic_lights:
		the_trafficlight = TrafficLight(GlobalSim.step, traffic_light_id, rsu_location, rsu_location)
		TL_edges[traffic_light_id] = the_trafficlight
		the_trafficlight.set_edges()
	
def run_simulation() -> None:
	global Infras, TL_edges
	...
	traci.init(1337)
	# Infras["CCTV"] = CCTV(GlobalSim.step, "CCTV", rsu_location, (0,0), "E1")
	get_tl()
	...
	for tl_id in TL_edges:
		tl = TL_edges[tl_id]
		print(f"max_waiting_time: {tl.debug_max_waiting_time}")
	traci.close()
